  ...
  var Board = {
    ...
    drawBoard: function() {
      ...
      for (var i = 0; i < 8; i++) {
        for (var j = 0; j < 8; j++) {
          ...
          if (this.board[i][j]) {
            ...
            this.ctx.beginPath();
            this.ctx.arc(x + (BOARD_SIZE/16), y + (BOARD_SIZE/16), (BOARD_SIZE/16) - (BOARD_SIZE/64), 0, 2 * Math.PI);
            this.ctx.stroke();
            this.ctx.fill();
      ...
      if (this.selected) {
        ...
        var moves = this.getPossibleMoves(this.selected.x, this.selected.y, true, this.board[this.selected.x][this.selected.y].isKing);
    ...
    handleCanvasClick: function(e) {
      ...
      if (this.board[board_x][board_y] && this.board[board_x][board_y].color == 'red') {
        ...
        if (this.selected) { 
          ...
          if (selectedMove) {
            this.executeMove(this.selected.x, this.selected.y, selectedMove, true);
            this.moved = true;
          }
          this.selected = null;
        }
    ...
    executeMove: function(x, y, move, human) {
      var p = this.board[x][y] 
      this.board[move.x][move.y] = p;
      this.board[x][y] = null;

      if (move.victims) {
        for (var i = 0; i < move.victims.length; i++) {
          this.board[move.victims[i].x][move.victims[i].y] = null;
        }
      }
      ...
      for (var i = 0; i < 8; i++) {
        for (var j = 0; j < 8; j++) {
          if (this.board[i][j] && this.board[i][j].color == 'white') {
            var moves = this.getPossibleMoves(i, j, false, this.board[i][j].isKing);
            if (moves.length > 0) {
              this.executeMove(i, j, moves[0], false);
              this.drawBoard();
              return;
            }
          }
    ...
    getPossibleMoves: function(x, y, human, king) {
      ...
      var attackMoves = this.getPossibleAttackMoves(x, y, human, king);
